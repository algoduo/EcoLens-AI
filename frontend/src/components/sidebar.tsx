...
// Uses the @ alias to avoid relative path traps
// Inside src/components/sidebar.tsx
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="flex h-16 items-center gap-3 border-b border-slate-100 px-6">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-emerald-500 shadow-sm">
          ...
          <div className="border-b border-slate-100 p-6 space-y-4">
            <div className="flex items-center gap-3">
              <Avatar className="h-12 w-12 border-2 border-emerald-500/20">
                ...
                <AvatarFallback className="bg-emerald-100 text-emerald-700 font-bold">
                  ...
                  {user?.displayName ? user.displayName.substring(0, 2).toUpperCase() : "JD"}
                </AvatarFallback>
              </Avatar>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold truncate text-slate-900">
                  {loading ? "Syncing..." : user?.displayName || "Eco Warrior"}
                </p>
            ...
            {/* Dynamic Eco Level Stats */}
            <div className="space-y-3 rounded-xl bg-slate-50 p-4 border border-slate-100">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-slate-500 uppercase">Eco Level</span>
                <Badge className="bg-emerald-500 hover:bg-emerald-600 border-none px-2 py-0">
                  Level {user?.ecoLevel || 1}
                </Badge>
              </div>
              ...
              <p className="text-[11px] text-slate-600">
                <span className="font-bold text-emerald-700">{user?.points?.toLocaleString() || 0}</span> points earned
              </p>
            </div>
          </div>
          ...
          <div className="border-t border-slate-100 p-4 space-y-2">
            <div className="flex items-center justify-between text-[10px] px-2">
              ...
              <span className="font-bold text-slate-900">#247</span>
            </div>
            <Button 
              variant="ghost" 
              size="sm"
      ...
      {/* Mobile Overlay */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-30 bg-slate-900/20 backdrop-blur-sm lg:hidden" onClick={() => setMobileMenuOpen(false)} />
      )}
    </>
  )
}